                        ...
                        <tbody>
                            @foreach ($data_lokasi_penelitian as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        @if ($item->status == '0')
                                            <span class="label label-warning">Menunggu Set SK dan Tanggal</span>
                                        @elseif($item->status == '1')
                                            <span class="label label-success">Disetujui oleh Fakultas</span>
                                        @elseif($item->status == '2')
                                            <span class="label label-info">Approve Wakil Dekan</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if ($item->status == '0')
                                            <button type="button" class="btn btn-primary" data-toggle="modal"
                                                data-target="#requestModal" data-id="{{ $item->id }}"
                                                data-nama="{{ $item->nama_pemohon }}" data-nim="{{ $item->nim_pemohon }}">
                                                Set SK dan Tanggal
                                            </button>
                                        @else
    ...
    <div class="modal fade" id="requestModal" tabindex="-1" role="dialog" aria-labelledby="requestModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form method="POST" action="{{ route('update_fakultas_request_surat_lokasi_penelitian') }}">

                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="requestModalLabel">Request Surat Lokasi Penelitian</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="nomor_sk" id="modal-sk" required>
                        </div>
                        <div class="form-group">
                            <label for="modal-tanggal">Tanggal</label>
                            <input type="date" class="form-control" name="tanggal" id="modal-tanggal" required>
                        </div>
