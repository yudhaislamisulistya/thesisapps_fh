                ...
                <div class="table-responsive">
                    <div class="row" style="margin-bottom: 10px">
                        ...
                        <div class="col-md-6 text-right">
                            <a href="{{ route('get_fakultas_riwayat_sk_pengusulan') }}" class="btn btn-info">Riwayat</a>
                        </div>
                    </div>
                    <form method="post" action="{{ route('post_fakultas_sk_pengusulan') }}">
                        {{ csrf_field() }}
                            ...
                            <tbody>
                                @foreach ($penetapan_pengusulan as $key => $value)
                                    <tr class="odd gradeX">
                                        ...
                                        <td><input type="checkbox" class="data_usulan" name="data[{{ $key - 1 }}]"
                                                value="{{ $value->C_NPM }}" onchange="checking(this)"></td>
                                        <td>{{ $value->C_NPM }}</td>
                                        ...
                                        <td class="text-center"><a
                                                href="{{ url('fakultas/set_pembimbing') }}/{{ $value->C_NPM }}/2"><i
                                                    class="fa fa-pencil icon-square icon-xs icon-primary"></i></a></td>
                                        <td><a class="btn btn-danger"
                                                href="{{ url('fakultas/batal_set_pembimbing') }}/{{ $value->C_NPM }}"><i
                                                    class="fa fa-trash"></i>Batal</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        <p align="left">
                            <button id="save" class="btn btn-primary btn-perspective" type="submit" disabled>Set SK
                                Pengusulan Pembimbing</button>
                        </p>
                    </form>
