...
@section('isi')
    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            {{-- data_pendaftaran cek apakah sudah pernah mendaftar atau tidak, jika sudah mendaftar tampilkan alert --}}
            @if ($status_pengajuan != null)
                <div class="alert alert-danger alert-block square fade in alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                    <p><strong>Status!</strong></p>
                    <p>Anda sudah mendaftar pada periode ini<i
                            ...
                            <tbody>
                                @php
                                    ...
                                    $trtsyaratujian = \App\TrtSyaratUjian::where([
                                        ...
                                        ->whereIn(
                                            'syarat_ujian_id',
                                            \App\Model\mst_syarat_ujian::where(['tipe_ujian' => 0])->select(
                                                'syarat_ujian_id',
                                            ),
                                        )
                                        ->count();
                                    $trtreg = \App\Model\trt_reg::whereIn(
                                        ...
                                        ->whereIn(
                                            'pendaftaran_id',
                                            \App\Model\mst_pendaftaran::where('tipe_ujian', 0)->select(
                                                'pendaftaran_id',
                                            ),
                                        )
                                        ->count();
                ...
                <div style="display: flex">
                    @php
                        $trtsyaratujianx = \App\TrtSyaratUjian::where('C_NPM', auth()->user()->name)
                            ->whereIn(
                                'syarat_ujian_id',
                                \App\Model\mst_syarat_ujian::where('tipe_ujian', 0)->select('syarat_ujian_id'),
                            )
                            ->count();
                    ...
                    @if (count($syarat) == $trtsyaratujianx && count($syarat) != 0)
                        @if ($trtpengajuandokumen == 0)
                            <button data-href="{{ url('/mhs/ajukan_dokumen/0') }}" onclick="showModal(this)"
