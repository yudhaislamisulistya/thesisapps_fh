							...
							<div class="row">
								<div class="col-sm-4 col-md-3">
									<div class="list-group success square no-border">
									  <a href="{{ url('mhs/mail_inbox')}}" class="list-group-item active">Pesan Masuk <span class="badge badge-success">{{helper::getDataPesanMasuk(auth()->user()->name)}}</span></a>
									  <a href="{{ url('mhs/mail_sent')}}" class="list-group-item">Pesan Keluar <span class="badge badge-primary">{{helper::getDataPesanKeluar(auth()->user()->name)}}</span></a>
									</div>
								...
								<div class="col-sm-8 col-md-9">
									...
									@foreach ($data as $key => $value)
									  <a href="{{ url('mhs/mail_read/'.$value->pesan_id.'/1')}}" class="list-group-item mail-list">
										<input type="checkbox" class="i-grey-flat">
										  @if($value->status_baca == "0")
											  <i class="fa fa-circle text-primary"></i>
										  @endif
										<span class="name">{{ helper::getDeskripsi($value->pengirim_id)}}</span>
