    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="the-box">
                ...
                @else
                    @php
                        ...
                        $bidangilmuid = \App\RequestPembimbing::where([
            ...
            <div class="the-box">
                @if ($trtbimbingan->isEmpty())
                    @php
                        $statusKonfirmasiTopikPenelitian = helper::getStatusKonfirmasiTopikPenelitian(
                            Auth::user()->name,
                        );
                    @endphp
                                    ...
                                    <select class="form-control bold-border" name="pembimbing_I_id"
                                        ...
                                        @foreach ($listdosen as $key => $value)
                                            @if ($value->level == '1' || !isset($value->level) || $value->level == '3')
                                                <option value="{{ $value->C_KODE_DOSEN }}">{{ $value->NAMA_DOSEN }}
                                                </option>
                                            @endif
                                        @endforeach
                                    </select>
                ...
                @else
                    @php
                        $pembimbing1 = Illuminate\Support\Facades\DB::table('t_mst_dosen')
                            ->where('C_KODE_DOSEN', $trtbimbingan[0]->pembimbing_I_id)
                            ->pluck('NAMA_DOSEN')[0];
                        $pembimbing2 = Illuminate\Support\Facades\DB::table('t_mst_dosen')
                            ->where('C_KODE_DOSEN', $trtbimbingan[0]->pembimbing_II_id)
                            ->pluck('NAMA_DOSEN')[0];
                    @endphp
                    <fieldset>
                        <div class="form-group">
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td>
                                        <button class="btn btn-primary" onclick="showModal(this)"
                                            data-target="#modalDefault" data-toggle="modal"
                                            data-href="{{ asset('dokumen/' . $value->kerangka) }}" target="_blank"><i
                                    ...
                                    @if ($trtbimbingan->isEmpty())
                                        ...
                                        @else
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-info"
                                                    href="{{ url('mhs/ubah_judul') }}/{{ $value->topik_id }}"><i
