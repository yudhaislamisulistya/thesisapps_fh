        ...
        <div class="container-fluid">
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-th-block">
                            ...
                            @php
                                ...
                                $trtsyaratujian = \App\TrtSyaratUjian::where(["C_NPM" => auth()->user()->name, "status" => 1])->whereIn("syarat_ujian_id", \App\Model\mst_syarat_ujian::where(["tipe_ujian" => 1])->select("syarat_ujian_id"))->count();
                                $trtreg = \App\Model\trt_reg::whereIn("bimbingan_id",\App\Model\trt_bimbingan::where("C_NPM", auth()->user()->name)->select("bimbingan_id"))->whereIn("pendaftaran_id",\App\Model\mst_pendaftaran::where("tipe_ujian", 1)->select("pendaftaran_id"))->count();
                            @endphp
                            @foreach ($data as $key => $value)
                                <tr class="odd gradeX">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            @if($mstsyaratujian == $trtsyaratujian && !empty($mstsyaratujian) && $value->jml_peserta < $value->kuota && empty($trtreg))
                                                <button class="btn btn-primary btn-perspective" type="submit" name="pendaftaran_id" value="{{$value->pendaftaran_id}}">Daftar</button>
                                            @else
                                                <button class="btn btn-primary btn-perspective" type="button" disabled>Daftar</button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="the-box">
                    ...
                    <form action="{{url("mhs/syarat_ujianpost")}}" onsubmit="return showPostModal(this)" method="post">
                        ...
                        <table class="table table-striped table-hover" id="">
                            ...
                            @foreach ($syarat as $key => $value)
                                @php
                                    $trtsyaratujian = \App\TrtSyaratUjian::where(["syarat_ujian_id" => $value->syarat_ujian_id, "C_NPM" => auth()->user()->name])->first()
                                @endphp
                                <tr class="odd gradeX">
                                    <td width="1%" align="center">{{++$key}}</td>
                                    ...
                                    <td>
                                        <div class="col-lg-5">
                                            <input type="hidden" name="syarat_ujian_id[]" value="{{$value->syarat_ujian_id}}"/>
                                            @if($key == 1)
                                                <input type="hidden" name="sui"/>
                                            @endif
                                            <input type="text" class="form-control bold-border" name="link[]" value="{{empty($trtsyaratujian) ?"": $trtsyaratujian->link}}"/>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @if(!empty($trtsyaratujian))
                                            <button type="button" value="{{$key-1}}" onclick="showPostModal(this)" data-formaction="{{url("mhs/syarat_ujianpost")}}" data-target="#modalInfo" data-toggle="modal" class="btn btn-info"><i class="fa fa-edit"></i></button>
                                            <button type="button" onclick="showModal(this)" data-href="{{ url("mhs/syarat_ujiandel/1/$value->syarat_ujian_id")}}" data-target="#modalDanger" data-toggle="modal" class="btn btn-danger"><i class="fa fa-trash"></i></button>
                                        @else
                                            <button type="button" value="{{$key-1}}" onclick="showPostModal(this)" data-formaction="{{url("mhs/syarat_ujianpost")}}" data-target="#modalPrimary" data-toggle="modal" class="btn btn-primary"><i class="fa fa-save"></i></button>
                                        @endif
                                    </td>
                                    <td>
                                        @if(!empty($trtsyaratujian))
                                            <button type="button" onclick="showModal(this)" data-href="{{$trtsyaratujian->link}}" data-target="#modalDefault" data-toggle="modal" class="btn bg-dark" style="color: #fff"><i class="fa fa-paperclip"></i></button>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div style="display: flex">
                    @php
                        $trtsyaratujianx = \App\TrtSyaratUjian::where("C_NPM",auth()->user()->name)->whereIn("syarat_ujian_id", \App\Model\mst_syarat_ujian::where("tipe_ujian",1)->select("syarat_ujian_id"))->count();
                        $trtpengajuandokumen = \App\TrtPengajuanDokumen::where(["C_NPM" => auth()->user()->name, "type" => 1])->count();
                    @endphp
                    @if(count($syarat) == $trtsyaratujianx && count($syarat) != 0)
                        @if($trtpengajuandokumen == 0)
                            <button data-href="{{url("/mhs/ajukan_dokumen/1")}}" onclick="showModal(this)" data-target="#modalWarning" data-toggle="modal" class="btn btn-warning btn-perspective" style="margin-left: auto">Ajukan</button>
                        @else
                            <button data-href="{{url("/mhs/ajukan_dokumen/1")}}" onclick="showModal(this)" data-target="#modalDanger1" data-toggle="modal" class="btn btn-danger btn-perspective" style="margin-left: auto">Batalkan ajuan</button>
                        @endif
                    @else
